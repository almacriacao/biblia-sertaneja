...
interface LibraryProps {
  songs: Song[];
  playlists: Playlist[];
  selectedAlbum?: Album | null; // New Prop to show an album directly
  onClearSelectedAlbum?: () => void; // Callback to close album view
...
export const Library: React.FC<LibraryProps> = ({ 
  ...
  // Effect to handle external album selection (from Home)
  useEffect(() => {
    if (selectedAlbum) {
      // Convert Album to a temporary "Playlist" object for viewing
      const albumAsPlaylist: Playlist = {
        id: selectedAlbum.id,
        title: selectedAlbum.title,
        description: `Álbum • ${selectedAlbum.author} • ${selectedAlbum.year}`,
      ...
      // If cleared externally (optional)
  ...
  const handleBack = () => {
    setSelectedPlaylist(null);
    if (onClearSelectedAlbum) onClearSelectedAlbum();
  };

  // Virtual Playlist for Favorites
  const likedSongsPlaylist: Playlist | null = favoriteSongs.size > 0 ? {
    id: 'liked-songs',
    title: 'Músicas Curtidas',
    description: 'Seus louvores favoritos em um só lugar.',
    coverUrl: 'https://images.unsplash.com/photo-1493225255756-d9584f8606e9?auto=format&fit=crop&q=80&w=300&h=300', 
  ...
  const renderPlaylistView = (playlist: Playlist) => {
    ...
    const isLikedPlaylist = playlist.id === 'liked-songs';
    
    return (
      <div className="h-full overflow-y-auto pb-32 scrollbar-hide">
         <div className={`p-8 flex flex-col md:flex-row gap-8 items-end ${isLikedPlaylist ? 'bg-gradient-to-b from-purple-800 to-black' : 'bg-gradient-to-b from-zinc-800 to-black'}`}>
            <div className={`w-52 h-52 shadow-2xl shadow-black flex items-center justify-center ${isLikedPlaylist ? 'bg-gradient-to-br from-indigo-500 to-purple-600' : ''}`}>
               {isLikedPlaylist ? (
                 <Icon name="heart-filled" className="w-20 h-20 text-white" />
               ) : (
            ...
            <div className="flex-1">
               <p className="text-xs font-bold uppercase text-white mb-2">
                 {isLikedPlaylist ? 'Playlist' : (playlist.isUserCreated ? 'Playlist Pessoal' : (selectedAlbum ? 'Álbum' : 'Playlist Oficial'))}
               </p>
               <h1 className="text-4xl md:text-6xl font-black text-white mb-4 tracking-tighter">{playlist.title}</h1>
               <p className="text-zinc-300 mb-4 text-sm font-medium">{playlist.description}</p>
         ...
         <div className="p-8 bg-black/20">
            <div className="flex items-center justify-between mb-8">
               ...
               {playlist.isUserCreated && (
                 <button onClick={() => { onDeletePlaylist(playlist.id); handleBack(); }} className="text-zinc-500 hover:text-red-500" title="Apagar Playlist">
                    ...
                    >
                      ...
                      <td className="py-3">
                        <div className="flex items-center gap-3">
                          <img src={song.coverUrl} className="w-10 h-10 grayscale-0 object-cover rounded" />
                          <div>
                      ...
                      <td className="py-3 text-right font-mono pr-2">
                         <div className="flex items-center justify-end gap-3">
                            <button 
                                ...
                                className={`hover:scale-110 transition ${isFav ? 'text-green-500' : 'text-zinc-600 hover:text-white opacity-0 group-hover:opacity-100'}`}
                            >
                                <Icon name={isFav ? "heart-filled" : "heart"} className="w-4 h-4" />
                            </button>
  ...
  let displayedPlaylists = isOffline 
    ? playlists.filter(p => isPlaylistAvailableOffline(p))
    : playlists;

  return (
    <div className="p-8 pb-32 overflow-y-auto h-full bg-gradient-to-b from-zinc-900 to-black scrollbar-hide">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
          {/* Liked Songs Card (Dynamic) */}
          {likedSongsPlaylist && !isOffline && (
            <div 
              onClick={() => setSelectedPlaylist(likedSongsPlaylist)}
              className="bg-gradient-to-br from-purple-800 to-indigo-900 p-4 rounded-lg hover:brightness-110 transition cursor-pointer group relative"
            >
               <div className="mb-4 w-full aspect-square flex items-center justify-center">
                   <div className="flex flex-col items-center gap-2">
                     <Icon name="heart-filled" className="w-16 h-16 text-white" />
                   </div>
                   <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition rounded-md flex items-center justify-center">
               ...
               <h3 className="font-bold text-white mb-1 truncate">Músicas Curtidas</h3>
               <p className="text-sm text-purple-200">{likedSongsPlaylist.songs.length} músicas favoritas</p>
            </div>
          )}
          ...
          {displayedPlaylists.length > 0 ? (
            displayedPlaylists.map(playlist => (
             ...
             (!likedSongsPlaylist && <p className="col-span-full text-zinc-500">Sua biblioteca está vazia.</p>)
          )}
      </div>
    </div>
  );
